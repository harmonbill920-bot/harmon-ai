...
def predict_match(home_id, away_id, fixture_id=None, league_id=None, season=None):
    ...
    h, a, head = hf["form"], af["form"], hh["h2h"]
    ha, aa = h["goals_per_game"] or 1.15, a["goals_per_game"] or 1.0
    hd, ad = h["conceded_per_game"] or 1.1, a["conceded_per_game"] or 1.1
    hx = max(.2, min(3.8, .48*ha + .28*ad + .24*1.05))
    ax = max(.15, min(3.5, .48*aa + .28*hd + .24*.9))
    if head["matches"]:
        total = (head["home_goals"] + head["away_goals"]) / head["matches"]
        hx = .85*hx + .15*max(.1, total*.55)
        ax = .85*ax + .15*max(.1, total*.45)
    p = {"home":0,"draw":0,"away":0}
    for i in range(7):
        for j in range(7):
            v = poisson(i,hx)*poisson(j,ax)
            p["home" if i>j else "draw" if i==j else "away"] += v
    total = sum(p.values()) or 1
    hp, dp, ap = round(p["home"]/total*100,1), round(p["draw"]/total*100,1), round(p["away"]/total*100,1)
    lam = hx + ax
    over15 = 1-poisson(0,lam)-poisson(1,lam)
    over25 = 1-sum(poisson(k,lam) for k in range(3))
    under25 = 1-over25
    over35 = 1-sum(poisson(k,lam) for k in range(4))
    btts = 1-math.exp(-hx)-math.exp(-ax)+math.exp(-lam)
    dc1x, dcx2, dc12 = hp+dp, dp+ap, hp+ap
    markets = {"Home Win":hp,"Draw":dp,"Away Win":ap,"Over 1.5":round(over15*100,1),
               "Over 2.5":round(over25*100,1),"Under 2.5":round(under25*100,1),
               "Over 3.5":round(over35*100,1),"BTTS Yes":round(btts*100,1),
               "BTTS No":round((1-btts)*100,1),"1X":round(dc1x,1),
               "X2":round(dcx2,1),"12":round(dc12,1)}
    best = max(markets, key=markets.get)
    conf = round(min(95,max(45,max(hp,dp,ap)*.72+max(0,min(1,(h["matches"]+a["matches"])/20))*28)),1)
    pred = "Home Win" if hp>=dp and hp>=ap else ("Away Win" if ap>=hp and ap>=dp else "Draw")
    ...
    out={"home_probability":hp,"draw_probability":dp,"away_probability":ap,"predicted_result":pred,
         "confidence":conf,"home_xg":round(hx,2),"away_xg":round(ax,2),
         "expected_goals":{"home":round(hx,2),"away":round(ax,2),"total":round(lam,2)},
         "over_under":{"over_1_5":round(over15*100,1),"over_2_5":round(over25*100,1),
                       "under_2_5":round(under25*100,1),"over_3_5":round(over35*100,1)},
         "btts":{"yes":round(btts*100,1),"no":round((1-btts)*100,1)},
         "double_chance":{"1X":round(dc1x,1),"X2":round(dcx2,1),"12":round(dc12,1)},
         "strongest_market":{"market":best,"probability":markets[best]},
